...
## Validation Gate for Agent Edits

If this agent's system prompt is modified, regenerate agent projections before running tests:
...
For Python 3.13 / libexpat conflict on macOS, use the fallback:
...
### Grep Usage in Verification Commands

**Use `grep -E` for wildcard verification:**

- File paths with variable components: `grep -E 'file_.*\.py'`
- Patterns with alternation: `grep -E '(pattern1|pattern2)'`
- Character classes: `grep -E '[a-z]+_[0-9]+'`

**Use `grep -F` for literal-string checks:**

- Exact function names: `grep -F 'my_function('`
- Exact error messages: `grep -F 'Not found'`
- Special characters: `grep -F -- '--flag'` (remember the `--` separator)

**When returning verification commands to the planner, use the appropriate flag.** Wildcard patterns require `-E`; exact literals require `-F`.
