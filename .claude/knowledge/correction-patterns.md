...
| A task uses `git add -u` or `git add <dir>` in a dirty worktree | Path-scope staging to the exact task paths; for `docs/health/` ledger tasks add a Step 0 that commits any pre-existing dirty health files first, so a later `git add docs/health/` only stages this task's regenerated output |
