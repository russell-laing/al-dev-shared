...
| Repo-local maintainer tooling is not treated as distributed plugin surface | Ownership boundary | `AGENTS.md`; `CODEX.md`; `docs/projection-layer-readme.md` | Manual generated-output review | `guide only` | Repo-local maintainer tooling | Add a deterministic boundary test before describing this as enforced. Keep `.claude`, `.codex`, local plans, and maintainer docs out of generated plugin expectations. |
