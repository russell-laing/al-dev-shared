...
**Goal:** Create the `al-dev-consolidate` skill that consolidates `.dev/` workflow artifacts into per-session vault-ready summary notes and a sessions index.

**Architecture:** A standalone 5-phase skill (Phases 0–4) with no spawned agents. All content is extracted via bash commands — file content is never read into LLM context. Phase 0 handles resume/staleness logic; Phases 1–4 discover, classify, extract, and write session summaries and a sessions index to `.dev/sessions/`.

**Tech Stack:** Markdown skill file (SKILL.md), bash extraction commands, python3 one-liner for repo-name resolution, Obsidian wiki-link syntax in index output.
...
**Files:**
- Create: `profile-al-dev-shared/skills/al-dev-consolidate/SKILL.md`

- [ ] **Step 1: Create the skill directory**

```bash
mkdir -p profile-al-dev-shared/skills/al-dev-consolidate
```

- [ ] **Step 2: Write SKILL.md**

Write `profile-al-dev-shared/skills/al-dev-consolidate/SKILL.md` with the following content (use the Write tool):
...
```bash
ls -la profile-al-dev-shared/skills/al-dev-consolidate/SKILL.md
wc -l profile-al-dev-shared/skills/al-dev-consolidate/SKILL.md
```

Expected: file exists, line count > 200.

- [ ] **Step 4: Verify all five phases are present**

```bash
grep '^## Phase' profile-al-dev-shared/skills/al-dev-consolidate/SKILL.md
```

Expected: exactly 5 lines (Phase 0 through Phase 4).

- [ ] **Step 5: Verify all four group extraction blocks are present**

```bash
grep -c 'Group [ABCD]' profile-al-dev-shared/skills/al-dev-consolidate/SKILL.md
```

Expected: at least 8 matches (multiple references to each group).

- [ ] **Step 6: Forbidden pattern scan**
...
Expected: no output. Note: `YYYY-MM-DD` appears intentionally as a template
placeholder for output path examples inside code blocks — this is correct
and expected for a skill file (it is the naming convention, not an unrendered
date). The forbidden pattern check does not apply to `YYYY-MM-DD` in SKILL.md
files.

- [ ] **Step 7: Commit**

```bash
git add profile-al-dev-shared/skills/al-dev-consolidate/SKILL.md
git commit -m "feat(skill): add al-dev-consolidate artifact consolidation skill"
```

---

### Task 2: Update the plugin map

**Files:**
- Modify: `docs/al-dev-plugin-map.md`

- [ ] **Step 1: Update the skill count in the header**

In `docs/al-dev-plugin-map.md`, find and replace the `Last updated` line:

Old:
```
**Last updated:** 2026-05-27 (18 distributed skills: 17 primary + 1 deprecated alias skill,
```

New:
```
**Last updated:** 2026-05-28 (19 distributed skills: 18 primary + 1 deprecated alias skill,
```

- [ ] **Step 2: Add a Layer 2 section for /al-dev-consolidate**

In `docs/al-dev-plugin-map.md`, find the exact line:
...
Insert the following block immediately before that line (keeping the `---` separator before `## Observations`):
...
- [ ] **Step 3: Add to "Skills with no dedicated agent" list**
...
Append a line immediately after it:
...
```bash
grep 'Last updated' docs/al-dev-plugin-map.md
grep -c 'al-dev-consolidate' docs/al-dev-plugin-map.md
```

Expected: header shows `19 distributed skills`; at least 3 occurrences of
`al-dev-consolidate` in the map.

- [ ] **Step 5: Forbidden pattern scan**

```bash
grep -n '\[date\]\|TODO\|TBD\|Co-Authored-By\|claude:\|copilot:' \
  docs/al-dev-plugin-map.md | head -5
```

Expected: no new matches introduced by this task (existing content may have
some — compare against pre-task count if uncertain).

- [ ] **Step 6: Commit**

```bash
git add docs/al-dev-plugin-map.md
git commit -m "docs(map): add al-dev-consolidate to plugin map as utility skill"
```
