...
| align-harness-repos | Bloat: Phase 5 token-replacement block >30 lines inline | fixed | 2026-06-07 | `95b10b3` — extracted replacement rules to `.claude/knowledge/harness-token-map.md`; closes row 165 |
| review-documentation-map | Bloat: Phase 2 (~76 lines) profile extraction + caller sets inline | fixed | 2026-06-07 | `95b10b3` — extracted profile schema and comparison rules to two knowledge files; closes row 166 |
| record-health-dispositions | Clarity: no rule for contradictory batch input | fixed | 2026-06-07 | `95b10b3` — added the contradictory-batch guard before row writes; closes row 167 |
| plan-health-findings | Clarity: --skills/--agents and FILTER_TYPE interaction order not stated | fixed | 2026-06-07 | `95b10b3` — object-type routing now runs before finding-type filtering; closes row 169 |
| design-agent-lens-caller-alignment | Clarity: "passed context with no dispatch line" severity label missing | fixed | 2026-06-07 | `95b10b3` — context without a dispatch line is explicitly High severity; closes row 170 |
| design-skill-lens-near-duplicates | Clarity: "within 2 of each other" phase count comparison ambiguous | fixed | 2026-06-07 | `95b10b3` — comparison now says phase counts differ by at most 2; closes row 171 |
| audit-knowledge-quality | Bloat: Phase 2a and 2b near-identical decision trees differing only by file-count threshold | fixed | 2026-06-07 | `95b10b3` — unified path choice under one threshold-driven decision; closes row 172 |
| align-harness-repos | Clarity: Phase 5 fenced-code-block trigger condition for manual-review flag is vague | fixed | 2026-06-07 | `95b10b3` — fenced-code occurrences are explicitly manual-review only; closes row 173 |
| audit-knowledge-quality | Clarity: Phase 2a step 2 no else branch for orphaned knowledge file | fixed | 2026-06-07 | `95b10b3` — unreferenced files are now recorded as orphaned with LOW severity; closes row 174 |
| plan-health-findings | Clarity: "rubber-duck it by reading the live subject file in full first" — "in full" imprecise | fixed | 2026-06-07 | `95b10b3` — wording now requires reading the entire subject file from start to finish; closes row 175 |
| plugin-health-discover | Clarity: Phase 3.1b "false" Move findings undefined operationally | fixed | 2026-06-07 | `95b10b3` — tooling-surface Move suggestions are defined as non-actionable false positives; closes row 176 |
| review-documentation-map | Clarity: Phase 6 no else branch for "not accurate AND NO_UPDATE=false" | fixed | 2026-06-07 | `95b10b3` — the update path now explicitly proceeds to Phase 7; closes row 177 |
| sync-documentation-maps-collect | Clarity: Phase 2 no guard against infinite retry | fixed | 2026-06-07 | `95b10b3` — repeated pending state is treated as stalled and routes to workflow restart; closes row 178 |
| sync-documentation-maps-collect | Clarity: Phase 3 sort order and dedup winner unspecified | fixed | 2026-06-07 | `95b10b3` — stable sort and first-item dedup winner are now explicit; closes row 179 |
| sync-documentation-maps-write | Clarity: "If the three values differ, stop" has no recovery action | fixed | 2026-06-07 | `95b10b3` — added generated-section refresh and forced workflow restart recovery paths; closes row 180 |
| plugin-health-audit | Description: implies single-unit dispatch; body delegates to two sub-skills | fixed | 2026-06-07 | `95b10b3` — description now names the discover then report internal workflow; closes row 181 |
| sync-documentation-maps-apply | Name Fit: "apply" implies writing; validation gate invisible to callers | fixed | 2026-06-07 | Verified against live file 2026-06-07 — description and heading explicitly say validates artifacts before writing maps; closes row 182 |
| sync-documentation-maps-apply | Structure: argument-hint references RUN_ID; body describes --team-ids parameter | fixed | 2026-06-07 | `95b10b3` — argument hint now uses `--team-ids` and `--skip-commit`; closes row 183 |
| record-health-dispositions | Bloat: Closure write-back rule dated procedural tone | fixed | 2026-06-07 | `95b10b3` — moved background to the ledger closure protocol and retained a three-item checklist; closes row 185 |
| align-harness-repos | Clarity: Phase 5 <placeholder> meta-notation undefined | fixed | 2026-06-07 | Verified against live file 2026-06-07 — Phase 5 uses concrete finding tokens and the extracted map defines replacement behavior without unresolved placeholder syntax; closes row 186 |
| design-agent-lens-caller-alignment | Structure: code blocks lack language tags | fixed | 2026-06-07 | Verified against live file 2026-06-07 — no fenced code blocks remain in the agent; closes row 191 |
| quality-agent-lens-clarity | Structure: pseudo-code block lacks language tag | fixed | 2026-06-07 | Verified against live file 2026-06-07 — no fenced code blocks remain in the agent; closes row 192 |
| quality-skill-lens-clarity | Structure: pseudo-code block lacks language tag | fixed | 2026-06-07 | Verified against live file 2026-06-07 — no fenced code blocks remain in the agent; closes row 193 |
| align-harness-repos | Structure: code blocks in Phase 2-4 lack language tags | fixed | 2026-06-07 | Verified against live file 2026-06-07 — command and output fences are tagged `bash` or `text`; closes row 194 |
| audit-knowledge-quality | Structure: phase headers mix "Phase N" and "Step N" | fixed | 2026-06-07 | Verified against live file 2026-06-07 — workflow headings consistently use Phase terminology; closes row 196 |
| fix-knowledge-quality | Structure: code blocks lack language tags | fixed | 2026-06-07 | Verified against live file 2026-06-07 — fences are tagged `bash` and `text`; closes row 197 |
| plugin-health-discover | Structure: code blocks lack bash tags | fixed | 2026-06-07 | Verified against live file 2026-06-07 — command fences are tagged `bash` and output examples `text`; closes row 198 |
| plugin-health-report | Structure: code blocks lack language tags; phase header punctuation inconsistent | fixed | 2026-06-07 | Verified against live file 2026-06-07 — fences are tagged and phase headings use em-dash punctuation; closes row 199 |
| projection-sync | Structure: code blocks lack bash tags | fixed | 2026-06-07 | Verified against live file 2026-06-07 — all command fences are tagged `bash`; closes row 200 |
| record-health-dispositions | Structure: code blocks lack bash tags | fixed | 2026-06-07 | Verified against live file 2026-06-07 — command and row-template fences are tagged; closes row 201 |
| review-documentation-map | Structure: nested "### Phase 2b" under "## Phase 2" breaks header hierarchy | fixed | 2026-06-07 | `95b10b3` — replaced the nested phase with one flat Phase 2 extraction workflow; closes row 202 |
| review-maps | Structure: code block lacks bash tag | fixed | 2026-06-07 | Verified against live file 2026-06-07 — no fenced code blocks remain in the router skill; closes row 203 |
| sync-documentation-maps-apply | Structure: code blocks lack language tags | fixed | 2026-06-07 | Verified against live file 2026-06-07 — all command and output fences are tagged; closes row 204 |
| sync-documentation-maps-collect | Structure: code blocks lack language tags | fixed | 2026-06-07 | Verified against live file 2026-06-07 — all command and output fences are tagged; closes row 205 |
| plugin-health-discover | Bloat: 9 top-level sections; Phase 3 nested 3.1/3.1b/3.2/3.3 | fixed | 2026-06-07 | `45372b2`, `8ce7fa7`, `a9ef0b3` — consolidated dispatch, folded cadence into Phase 0, and removed the stub subsystem; closes row 65 |
| sync-documentation-maps-agent-update | Bloat: Instructions ~43 lines | fixed | 2026-06-07 | `e8efc32` — extracted the shared update procedure to `.claude/knowledge/sync-map-update-shared.md`; closes row 77 |
| sync-documentation-maps-skill-update | Bloat: Instructions ~43 lines | fixed | 2026-06-07 | `e8efc32` — extracted the shared update procedure to `.claude/knowledge/sync-map-update-shared.md`; closes row 79 |
| plugin-health-discover | Clarity: Phase 1 cadence guard missing else branch (happy path implicit) | fixed | 2026-06-07 | Added an explicit branch that proceeds to the stale-open check when current disposition coverage exists; closes row 168 |
| sync-documentation-maps | Bloat: "Abandoned runs spawn audit agents whose results are never read" explanatory sentence in Phase 0 | fixed | 2026-06-07 | Removed the explanatory sentence while retaining the operative checkpoint cadence guard; closes row 184 |
