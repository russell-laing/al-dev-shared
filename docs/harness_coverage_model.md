...
| Long-running workflows resume from compact durable state | Workflow behavior | `profile-al-dev-shared/knowledge/workflow-resilience.md`; `profile-al-dev-shared/skills/develop-orchestrate/tests/scenarios.yaml` | `develop-writes-resume-artifacts` scenario; manual resume review | `guide + test` | Shared profile | Add new rows only when additional resume artifacts become durable contracts. |
