# develop-orchestrate Developer Spawn Prompt

This document defines the structured prompt template used to dispatch
developer agents during `develop-orchestrate` Phase 3 (Spawn Developers).
...
only. Commits are handled separately by /commit after user
approval.
```
...
1. **Gather context** — read the solution plan,
   the latest `.dev/*-develop-checklist.md`, the latest
   `.dev/*-develop-scope.md`, and any planning symbol evidence.
