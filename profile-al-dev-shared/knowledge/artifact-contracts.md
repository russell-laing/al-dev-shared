...
| ------ | --- | --- | --- | --- | --- |
| `ticket` | Freshdesk ticket ID or URL, `.dev/` write access | `.dev/*-ticket-ticket-context.md` (metadata context), `.dev/*-plugin-support-reply-<slug>.md` (drafted reply, optional, full mode) | `.dev/*-ticket-ticket-context.md` (latest) | `.dev/*-ticket-ticket-context.md` | latest ticket context file exists, is non-empty, contains ticket metadata section (ID, status, priority, summary), and was read after write in the current run |
