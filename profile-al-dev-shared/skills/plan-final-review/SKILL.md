...
Validation and user approval gate for the solution plan written by /plan.

## Artifact Contract

| | |
|---|---|
| **Required inputs** | `.dev/*-plan-solution-plan.md` (written by /plan Phase 5) |
| **Optional inputs** | `.dev/plan-critique-*.md` (written by /plan-with-critics, when present) — critic findings to surface at the approval gate |
| **Durable outputs** | None — user approval is the terminal output |
| **Resume read order** | Locate latest `.dev/*-plan-solution-plan.md` |
| **Success evidence** | User selects Approve before this skill exits |

## Phase 1 — Locate solution plan

```bash
ls -t .dev/*-plan-solution-plan.md 2>/dev/null | head -1
```

If no solution plan found: stop and report "No solution plan found in .dev/. Run /plan first, then re-run /plan-final-review."
...
If `$CRITIQUE` is non-empty, read it and carry its findings into the approval summary (Phase 3). If absent, continue silently — critic findings are optional (the plan may not have run /plan-with-critics).
...
```bash
VALIDATOR="$AL_DEV_SHARED_PLUGIN_ROOT/skills/plan/validate-plan.py"
REQ=$(ls .dev/*-interview-requirements.md 2>/dev/null \
  | sort | tail -1)
...
```text
Solution plan complete -> .dev/$(date +%Y-%m-%d)-plan-solution-plan.md
