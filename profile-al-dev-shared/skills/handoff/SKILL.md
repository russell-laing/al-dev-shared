...
| Single-repo fix (root cause is here) | ❌ Use `/plan` |
...
| --- | --- |
| `$(ls .dev/*-ticket-ticket-context.md 2>/dev/null &#124; sort &#124; tail -1)` | `.dev/source-ticket-context.md` |
| `$(ls .dev/*-explore-findings.md 2>/dev/null &#124; sort &#124; tail -1)` | `.dev/source-explore-findings.md` |
| `.dev/project-context.md` | `.dev/source-project-context.md` |
| `$(ls .dev/*-plan-solution-plan.md 2>/dev/null &#124; sort &#124; tail -1)` | `.dev/source-solution-plan.md` |
| `$(ls .dev/*-interview-requirements.md 2>/dev/null &#124; sort &#124; tail -1)` | `.dev/source-requirements.md` |
...
`.dev/$(date +%Y-%m-%d)-handoff-handoff-prompt.md`:
...
~~~text
/plan [specific fix description based on confirmed root cause]
~~~

Or if more investigation of this repo is needed first:

~~~text
/investigate [specific question about this repo's code]
~~~
```

Write this to `.dev/$(date +%Y-%m-%d)-handoff-handoff-prompt.md`
...
Session prompt → .dev/[date]-handoff-handoff-prompt.md

To continue in [target repo name]:
1. Open a new session rooted at: [target-repo-path]
2. Paste the prompt from .dev/[date]-handoff-handoff-prompt.md
   (preview: cat .dev/[date]-handoff-handoff-prompt.md)
```
