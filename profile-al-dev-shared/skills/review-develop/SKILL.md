...
`knowledge/artifact-contracts.md` for `review-develop` has been
...
`.dev/$(date +%Y-%m-%d)-develop-code-review.md` — synthesized review findings from all three reviewers; this file is also the downstream handoff artifact named in `knowledge/artifact-contracts.md` and must be read before any final clean/ready claim.
...
1. **Create the code-review artifact:**
   ...
   ```bash
   CODE_REVIEW_FILE=".dev/$(date +%Y-%m-%d)-develop-code-review.md"
   ```
...
2. **Write the file** using this structure:
   ...
   **Compile:** ✅ Passed
   ...
   [READY: no blocking issues found — proceed to /commit]
   ```
...
1. **Display a summary** of the code-review artifact:
   ...
   Next step: Fix critical issues, then re-run /review-develop (or proceed directly to /commit if you accept the risk).
...
2. **Write acceptance-criteria-verified stamp (READY verdict only):**
   ...
   the AC re-read on the next `/commit` run:
