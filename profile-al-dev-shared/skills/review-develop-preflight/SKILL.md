---
name: review-develop-preflight
description: >-
  Pre-review qualification for review-develop. Locates the develop
...
Pre-review qualification for `/review-develop`. Run this first to locate
...
**Goal:** Find the Phase 4 handoff artifact written by `/develop-orchestrate`.
...
1. Search for the handoff artifact:
   ...
   ```bash
   ls .dev/*-develop-phase4-handoff.md 2>/dev/null | sort | tail -1
   ```
...
2. If no handoff artifact is found, stop and tell the user:
   ...
   Expected: .dev/*-develop-phase4-handoff.md
   Action required: Run /develop-orchestrate first and wait for it to complete Phase 4.
   ```

3. If `$ARGUMENTS` contains a path override (e.g. `/review-develop-preflight path/to/handoff.md`),
...
Run /review-develop now to dispatch reviewers.
```
...
Fix the issue above, then re-run /review-develop-preflight.
Do not run /review-develop until preflight reports "Prerequisites: all met".
```
