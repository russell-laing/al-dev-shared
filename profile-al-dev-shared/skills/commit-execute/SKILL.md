...
Load the approved commit plan written by `/commit-preflight`:
...
> `/commit-preflight` first to analyse staged changes and collect
> approval, then re-run `/commit-execute`."
...
> **Lint path ownership:** `commit` runs its own internal lint preflight
> via `commit-lint-fixer` (this phase). This is the canonical path for
> pre-commit lint validation within the commit workflow. `/lint` is a
...
(`commit-lint-fixer`); conditionally 3.2 (`commit-ooxml-validator`) when
...
- agent: `al-dev-shared:commit-lint-fixer`
...
- agent: `al-dev-shared:commit-executor`
...
```text
Agent tool:
  agent: al-dev-shared:commit-hook-classifier
...
```text
Agent tool:
  agent: al-dev-shared:commit-hook-fixer
...
- **`needs-manual-intervention`** — surface each `manual-review` failure
  ...
  the user resolves and re-runs `/commit-execute`.
