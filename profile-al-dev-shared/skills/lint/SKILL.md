...
> **When to use this skill vs. commit:** `/lint` is a standalone
...
> `commit` has its own internal lint preflight (`commit-lint-fixer`)
> and does not chain to this skill. Running `/lint` before `/commit`
...
`knowledge/artifact-contracts.md` for `lint` has been produced and read
for the current run.
...
Write `.dev/$(date +%Y-%m-%d)-lint-lint-report.md` with a
...
Full report → `.dev/$(date +%Y-%m-%d)-lint-lint-report.md`
```
