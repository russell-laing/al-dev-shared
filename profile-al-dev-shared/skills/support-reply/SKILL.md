...
name: support-reply
description: >-
  Research and draft customer replies for Freshdesk support tickets using
  ticket context plus BC and Microsoft sources. Produces a customer-facing
  reply draft and, with user confirmation and Freshdesk credentials, can
  post it back to the ticket.
...
```text
/support-reply .dev/2026-06-01-ticket-ticket-context.md
/support-reply                 — auto-detect latest ticket context
```

This skill is normally chained from `/ticket --mode=full`, but
...
2. **If no argument was passed**, auto-detect the latest context file:
   ...
   ```text
   No ticket context found. Run /ticket <id> first to load a
   ticket, then re-run /support-reply.
   ```
...
## Phase 2 — Dispatch support-reply-drafter (reply phase)
...
```text
Agent tool:
  agent: al-dev-shared:support-reply-drafter
