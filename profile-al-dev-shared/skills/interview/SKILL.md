...
Spawn a single **interview** agent to conduct the interview:
...
5. **GATE: The interview agent must explicitly state "INTERVIEW COMPLETE" and
   ...
   **Completion check:** Verify agent output contains `INTERVIEW COMPLETE` and a
   ...
   2. If still missing after re-dispatch: if all five mandatory categories were
      ...
      categories were covered, stop and tell the user to re-run /interview
      before planning.

## Phase 3: Validate and Log Interview Output

- [ ] **Step 3.1:** Read agent-generated requirements file (`.dev/$(date +%Y-%m-%d)-interview-requirements.md`) written in Phase 2

- [ ] **Step 3.2:** Verify file contains all required sections: Interview Questions, Answers, and Distilled Requirements

Run: `grep -q "## Interview Questions\|## Answers\|## Distilled Requirements" ".dev/$(date +%Y-%m-%d)-interview-requirements.md"`

Expected: All three section headers present

- [ ] **Step 3.3:** Append summary line to session-log (agent-created `.dev/session-log.md`)

Pattern: `- $(date): interview completed; requirements written to .dev/$(date +%Y-%m-%d)-interview-requirements.md`

Do NOT create `.dev/$(date +%Y-%m-%d)-interview-notes.md` (agent responsibility)
...
- [ ] **Step 3.4:** Validate the output:
   ...
   VALIDATOR="$AL_DEV_SHARED_PLUGIN_ROOT/skills/interview/validate-requirements.py"
   REQFILE=$(ls -t .dev/*-interview-requirements.md 2>/dev/null \
     | head -1)
...
Output: .dev/$(date +%Y-%m-%d)-interview-requirements.md
Questions asked: [N]
Key decisions: [N]
