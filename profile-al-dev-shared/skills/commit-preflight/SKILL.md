...
# `/commit-preflight` — Commit Analysis and Confirmation
...
You never run `git commit` yourself — always delegate via `/commit-execute`.
...
`commit` has been produced and read for the current staged state.
...
if [[ "$ARGUMENTS" =~ --ticket-id=([^ ]+) ]]; then
  ...
  # 2. Fall back to a prior /ticket context file
  TICKET=$(ls .dev/*-ticket-ticket-context.md 2>/dev/null | sort | tail -1)
  if [[ -n "$TICKET" ]]; then
    head -10 "$TICKET"
...
your changes, then run `/commit` again."
...
Run: `grep -E "^## Verdict|^Verdict:" .dev/*-develop-code-review.md | head -1`
...
Run: `grep -E "^## Findings|^Severity:" .dev/*-develop-code-review.md`
...
if [[ "$REVIEW_VERDICT" == "BLOCKING" ]]; then
  ...
  grep -A 20 "## Findings" .dev/*-develop-code-review.md | grep "CRITICAL\|HIGH"
  exit 1
fi
```
...
- agent: `al-dev-shared:commit-analyzer`
...
- **no** — run `git restore --staged <file>` for every deleted
  file, then **stop** (user must re-run `/commit` after
...
- agent: `al-dev-shared:commit-group-drafter` (model: haiku)
...
- `no` — stop; leave staged files unchanged (user must re-run `/commit` after reviewing staged files)
...
# Written by /commit-preflight — consumed by /commit-execute
...
Run /commit-execute to run lint preflight and execute the commits,
or /commit again to restart the full workflow.
```

Return normally. Do not invoke `/commit-execute` from here — the caller
(`/commit`) handles the handoff to execution after this skill returns, and a
standalone `/commit-preflight` run intentionally stops here so the user can
review the saved plan before executing.
