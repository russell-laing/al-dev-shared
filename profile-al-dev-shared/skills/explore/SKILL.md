...
| Situation | Use |
| ----------- | ----- |
| Before `/plan` — understand what already exists | ✅ |
| Before `/interview` — understand existing code context | ✅ |
...
- If `.dev/*-ticket-ticket-context.md` exists (from
  `/ticket`), read the latest:
...
`.dev/$(date +%Y-%m-%d)-explore-findings.md`.
...
.dev/2026-05-19-explore-findings.md
