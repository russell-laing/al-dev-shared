...
After all developers complete, /review-develop orchestrates the review panel,
...
`.dev/*-plan-solution-plan.md` must exist (from
...
Do not claim implementation is complete or ready for `/review-develop`
until the success evidence named in `knowledge/artifact-contracts.md` for
`develop-orchestrate` has been produced and read for the current run.
...
`/develop-orchestrate` maintains through the run so context compaction and session
...
- `.dev/$(date +%Y-%m-%d)-develop-progress.md` — dated session snapshot
- `.dev/$(date +%Y-%m-%d)-develop-checklist.md` — implementation checklist extracted from plan
- `.dev/$(date +%Y-%m-%d)-develop-scope.md` — file-level scope contract
...
2. Read the latest dated `*-develop-progress.md` if present
3. Read the latest dated `*-develop-checklist.md` if present
4. Read the latest dated `*-develop-scope.md` if present
...
1. Read the latest solution plan:
   `$(ls .dev/*-plan-solution-plan.md 2>/dev/null \
   | sort | tail -1)`
...
`.dev/$(date +%Y-%m-%d)-autonomous-signatures.md`
...
Write `.dev/$(date +%Y-%m-%d)-develop-checklist.md`
...
Write `.dev/$(date +%Y-%m-%d)-develop-scope.md` with:
...
```text
1. IF developer agent reports "scope incomplete" in output:
   ...
   - AND plan allows scope expansion → Add new tasks
     ...
     # "scope expansion permitted" marker or the user approved expansion in this session
...
ELSE IF developer agent reports "implementation complete":
   → Proceed to Phase 4 (verification)

ELSE IF developer agent reports "blocking issue":
...
Write to:
`.dev/$(date +%Y-%m-%d)-autonomous-static-validation.md`
...
2. Then dispatch `/review-develop` — only after reading the current
   Phase 4 handoff artifact and confirming it satisfies the `develop-orchestrate`
