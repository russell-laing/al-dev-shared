---
description: "Diagnose and recover from pre-commit hook failures. Analyzes hook error logs, identifies root causes, applies scripted fixes, and re-stages affected files. Never re-runs commits itself — returns next_step guidance so the caller re-dispatches the execute agent. Complements al-dev-commit-executor by handling the error path in isolation."
