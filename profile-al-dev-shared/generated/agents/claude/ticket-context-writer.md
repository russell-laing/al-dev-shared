...
| File | Description |
|------|-------------|
| `.dev/<date>-ticket-ticket-context.md` | Structured ticket context with fields, comments, metadata |
...
If `knowledge/ticket-image-patterns.md` is unavailable:

1. Fall back to basic detection: Look for `![]()` markdown syntax and base64 URIs
2. Do NOT fail; treat as "no images found" and continue
3. Report in agent summary: "Image detection degraded to basic patterns (knowledge file unavailable)"

### Step 2: Write Context File

Create `.dev/$(date +%Y-%m-%d)-ticket-ticket-context.md` following the structure defined in `knowledge/ticket-context-output-format.md`.
