...
description: "Draft a customer-facing reply from internal BC support research findings. Writes the combined findings + reply file. Dispatched by the al-dev-support-reply skill (reply phase). Pairs with support-researcher."
tools: ["edit"]
---
...
Take structured research findings from support-researcher and produce a clear, actionable customer reply. Write both the internal findings and the draft reply to a single `.dev/` file.
...
| RESEARCHER_FINDINGS | **Yes** | Full structured output block from support-researcher (passed as text block in dispatch prompt, not as a file path) |
...
**Step 3:** Write combined file following the structure defined in `knowledge/support-reply-output-format.md`.
