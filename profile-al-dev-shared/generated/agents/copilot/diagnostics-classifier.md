...
## Fixability Assessment Criteria

A violation is directly fixable if:

- Change does not require user decision (semantic, not stylistic)
- Fix can be applied via automated tool or regex
- Result passes syntax check without manual review

Examples:

- **Fixable:** Missing semicolon, type mismatch, deprecated API → direct replacement available
- **Judgment-Required:** Performance anti-pattern, naming convention violation → requires context assessment
