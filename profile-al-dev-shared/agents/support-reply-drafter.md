...
Take structured research findings from bc-support-researcher and produce a clear, actionable customer reply. Write both the internal findings and the draft reply to a single `.dev/` file.
...
| RESEARCHER_FINDINGS | **Yes** | Full structured output block from bc-support-researcher (passed as text block in dispatch prompt, not as a file path) |
