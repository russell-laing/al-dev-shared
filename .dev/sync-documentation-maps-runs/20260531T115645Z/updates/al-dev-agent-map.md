...
**Last updated:** 2026-06-01 (24 agents; added plan-map-changes-duck-worker + plugin-health-team missing from map)
...
**Model:** haiku
**Tools:** Bash, Write
**Spawned by:** /al-dev-ticket (fetch + attachment-download phases in `--mode=context-only` and `--mode=full`; `/al-dev-support` is a deprecated alias path to `--mode=full`)
...
| Input | Required | Description |
|-------|----------|-------------|
| `suggestion` | **Yes** | Single suggestion JSON object: `{ id, type, subject, proposed_change, target_files }` |
| `reference_doc_path` | **Yes** | Absolute path to `profile-al-dev-shared/knowledge/map-change-rubber-duck-checks.md` |
| `repo_root` | **Yes** | Repository root absolute path |
| `run_id` | **Yes** | Run identifier for organizing output |
| `manifest_path` | **Yes** | Absolute path to manifest file for coordination |

**Outputs:**

| Output | Description |
|--------|-------------|
| `.dev/plan-map-changes-runs/<run_id>/duck-records/<id>.json` | Duck record with verdict (ACCEPT/DEFER/REJECT) and evidence |
| `.dev/plan-map-changes-runs/<run_id>/duck-records/<id>-error.json` | Error record if unrecoverable failure occurs |

---

### plugin-health-team

**Description:** Remote agent orchestrator for parallel lens execution. Receives work queue, spawns 4-6 lenses per batch, manages progress, and writes results to repo-owned .dev/plugin-health-runs/ artifacts.
**Model:** opus
**Tools:** Bash, Read, Write
**Spawned by:** /plugin-health (referenced by plugin-health skill files; no active al-dev-shared: invocation found)

**Inputs:**

| Input | Required | Description |
|-------|----------|-------------|
| Work queue JSON | **Yes** | Lens definitions, file lists, and context — passed via skill dispatch |
| `run_id` | **Yes** | Run identifier for organizing output artifacts |

**Outputs:**

| Output | Description |
|--------|-------------|
| `.dev/plugin-health-runs/<run-id>/manifest.json` | Final state of all lenses (completed, pending, failed) |
| `.dev/plugin-health-runs/<run-id>/lens-results/<lens_name>.json` | One file per completed lens (success or failure) |
| Return block | Team ID and final status returned to calling session |
...
- **plan-map-changes-duck-worker** — used only by /plan-map-changes (skill reference is currently commented out)
- **plugin-health-team** — used only by /plugin-health (referenced by skill files; no active al-dev-shared: invocation found)

### Agents with no inputs/outputs documentation

None — all 24 agents have documented Inputs and Outputs tables.

### Potential shared agents

- **al-dev-developer-tdd** — used by /al-dev-develop and /al-dev-fix when a test plan exists
- **al-dev-developer-traditional** — used by /al-dev-develop, /al-dev-fix, and /al-dev-review-develop autonomous compile-fix loops when no test plan is used
