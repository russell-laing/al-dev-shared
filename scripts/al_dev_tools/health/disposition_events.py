...
def append_event(events_root: Path, event: dict[str, object]) -> Path:
    ...
    # Validate that all referenced closes_event_ids exist and don't reference self
    closes_ids = event.get("closes_event_ids", [])
    if isinstance(closes_ids, list):
        for close_id in closes_ids:
            close_id_str = str(close_id)
            if close_id_str == event_id:
                raise ValueError(f"event cannot reference itself in closes_event_ids: {close_id_str}")
            if close_id_str not in existing_ids:
