...
def docs_health_root(root: Path) -> Path:
    return root / DOCS_HEALTH


def dispositions_archived_root(root: Path) -> Path:
    return root / DISPOSITIONS_ARCHIVED


def dispositions_events_root(root: Path) -> Path:
    return root / DISPOSITIONS_EVENTS


def dispositions_history_root(root: Path) -> Path:
    return root / DISPOSITIONS_HISTORY


def compatibility_ledger_path(root: Path) -> Path:
    return root / DISPOSITIONS_LEDGER


def dispositions_open_view_path(root: Path) -> Path:
    return root / DISPOSITIONS_OPEN


def dispositions_current_view_path(root: Path) -> Path:
    return root / DISPOSITIONS_CURRENT


def dispositions_index_path(root: Path) -> Path:
    return root / DISPOSITIONS_INDEX


def dispositions_jsonl_migration_audit_path(root: Path) -> Path:
