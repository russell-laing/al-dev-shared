...
# Reuse the canonical dossier selector rather than re-globbing.
sys.path.insert(0, str(Path(__file__).resolve().parent))
from select_health_artifacts import select_artifacts  # noqa: E402
...
def run_list_open(root: Path) -> object:
    """Count open accepted events via the documented list-open CLI."""
    try:
...
def run_staleness(root: Path) -> object:
    """Parse legacy effective-open count from check_ledger_staleness.py."""
    try:
        result = subprocess.run(
        ...
        return NOT_AVAILABLE
    match = _EFFECTIVE_OPEN.search(result.stdout + result.stderr)
    return int(match.group(1)) if match else NOT_AVAILABLE
